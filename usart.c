...
void USART3_Init(void){
	...
	GPIO_InitStruct.GPIO_Pin = GPIO_Pin_10| GPIO_Pin_11; // Pins 6 (TX) and 7 (RX) will be used for USART3
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_AF; // GPIO pins defined as alternatefunction
	...
	GPIO_PinAFConfig(GPIOC, GPIO_PinSource10, GPIO_AF_USART3); // pins will automatically be assignedto TX/RX - refer to datasheet to see AF mappings
	...
	USART_InitStruct.USART_Mode = USART_Mode_Tx | USART_Mode_Rx; // enable both character transmit andreceive
	...
	USART_ITConfig(USART3, USART_IT_TC, ENABLE); // transmission completed event (for reseting TxReadyflag)
	USART_ITConfig(USART3, USART_IT_RXNE, ENABLE); // character received (to trigger buffering of newcharacter)
