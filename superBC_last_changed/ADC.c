...
void  ADC_IRQHandler(void) {
	...
	if(ADC_GetITStatus(ADC1, ADC_IT_JEOC) == SET){
		...
		if(k%2==0){
			Current.PRIM = ADC_GetInjectedConversionValue(ADC1, ADC_InjectedChannel_1)*3220/0xFFF;		// get value from ADC1 injected channel
			k++;
		}else{
			Current.SEC = ADC_GetInjectedConversionValue(ADC1, ADC_InjectedChannel_2)*3220/0xFFF;	
			k--;
		}
